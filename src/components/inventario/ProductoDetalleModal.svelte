<script lang="ts">
  ...
  // Existencias state
  let existenciasPorSucursal: Array<{
  ...
  async function generarQR(url: string) {
    try {
      ...
      QRcode = await QRCode.toDataURL(url, { width: 100, margin: 1 });
    } catch (err) {
  ...
  $: if (showModal && producto) {
    ...
    if (producto.direccionWeb) {
      generarQR(producto.direccionWeb);
    }
  ...
  async function cargarExistencias() {
    ...
    try {
      const data = await fetchSecure<
        Array<{ codigoAgencia: number; existencia: number; ubicacion: string }>
      >(`/api/existencias/${producto.id}`);
      existenciasPorSucursal = data ?? [];
    } catch (e) {
      existenciasPorSucursal = [];
      errorExistencias =
    ...
    >
      ...
      >
        ...
        <div class="pt-4 border-t border-slate-100">
          ...
          {:else if errorExistencias}
            <div class="text-center py-4 text-red-500 text-sm font-bold">
              {errorExistencias}
            </div>
