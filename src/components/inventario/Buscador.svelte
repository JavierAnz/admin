<script lang="ts">
  ...
  import type ProductoDetalleModalType from "./ProductoDetalleModal.svelte";
  ...
  let busqueda = "";
  let productosRaw: ProductoBuscador[] = [];
  let productosVisibles: ProductoBuscador[] = [];
  ...
  let ModalComponent: typeof ProductoDetalleModalType | null = null;
  ...
  function ordenarLista(lista: ProductoBuscador[]): ProductoBuscador[] {
    if (
      !filtros.precio &&
      !filtros.existencia &&
      !filtros.precioo &&
  ...
  function configurarObserver() {
    ...
    if (typeof window === "undefined" || limiteVisible >= productosRaw.length) return;

    observer = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting && limiteVisible < productosRaw.length) {
        limiteVisible = Math.min(limiteVisible + LOTE_INICIAL, productosRaw.length);
        productosVisibles = productosRaw.slice(0, limiteVisible);
      }
    }, { rootMargin: "400px" });
  ...
  function procesarResultados(data: ProductoBuscador[]) {
    productosRaw = ordenarLista(data);
    limiteVisible = LOTE_INICIAL;
    productosVisibles = productosRaw.slice(0, limiteVisible);
    setTimeout(() => configurarObserver(), 0);
  }

  async function abrirModal(producto: ProductoBuscador) {
    activeId = producto.id;
    if (!ModalComponent) {
      const modulo = await import("./ProductoDetalleModal.svelte");
      ModalComponent = modulo.default;
    }
    selProd = { ...producto };
    showModal = true;
  }
  ...
  async function realizarBusqueda() {
    ...
    try {
      ...
      const data = await fetchSecure<ProductoBuscador[]>(url, { signal: abortController.signal });
      ...
      errorMsg =
        error instanceof Error && error.message === "forbidden"
          ...
          : BRAND_CONFIG.copy.search.error;
    } finally {
      loading = false;
    }
  }
  ...
  function toggleFiltro(tipo: keyof typeof filtros, direccion: "asc" | "desc") {
    ...
    if (productosRaw.length > 0) {
      productosRaw = ordenarLista(productosRaw);
      productosVisibles = productosRaw.slice(0, limiteVisible);
    }
  }
...
>
  ...
  >
    ...
    {#if errorMsg}
      <p class="text-xs font-bold text-red-500 px-1">{errorMsg}</p>
    {/if}
    ...
    <div class="flex flex-wrap items-center justify-between gap-2 sm:gap-4">
      ...
      <div class="flex items-center gap-1 sm:gap-2">
        <span
          ...
          >{productosRaw.length} Items</span
        >
        {#if nombreAgencia}
  ...
  >
    ...
    {:else}
      ...
      {#if limiteVisible < productosRaw.length}
        <div bind:this={triggerElement} class="h-1 w-full opacity-0 pointer-events-none"></div>
      {/if}
    {/if}
  </div>
</div>

{#if showModal && ModalComponent && selProd}
  <svelte:component this={ModalComponent} bind:showModal producto={selProd} />
{/if}
        ...
        <div class="max-h-64 overflow-y-auto p-4 space-y-3">
          {#each listaCarrito as item}
            ...
            >
              <button
                type="button"
                on:click={() => abrirModal(item as ProductoBuscador)}
