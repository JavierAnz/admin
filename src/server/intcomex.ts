...
export async function getIntcomexData(query?: string): Promise<ProductoUniversal[]> {
    ...
    try {
        ...
        return (data || []).map((p: Record<string, unknown>) => ({
            id: `INT-${(p.Sku as string) || (p.Mpn as string)}`,
            nombre: (p.Description as string) || 'Sin nombre',
            existencia: (p.InStock as number) ?? 0,
            preciop: (p.Price as { UnitPrice?: number })?.UnitPrice ?? 0,
            precioa: (p.Price as { UnitPrice?: number })?.UnitPrice ?? 0,
            preciob: (p.Price as { UnitPrice?: number })?.UnitPrice ?? 0,
            marca: (p.Brand as string) || 'Genérico',
            modelo: p.Mpn as string,
            origen: 'EXTERNO',
            proveedorNombre: 'xxx',
            entregaInmediata: false,
            imagenUrl: p.ImageSquare as string,
        }));
    } catch (error) {
